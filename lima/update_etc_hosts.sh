...
# get list of ingresses from localk3sserver cluster
kubectlingresses=$(limactl shell localk3sserver1 kubectl get ingress -A --no-headers --ignore-not-found=true)
kubectlservices=$(limactl shell localk3sserver1 kubectl get svc -A --no-headers | awk '$5 !~ /<none>/ {print}')
